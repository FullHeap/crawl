...
import com.etoak.crawl.util.DateUtil;
import com.etoak.crawl.util.ExcelUtil;
import com.etoak.crawl.util.FileTool;
import com.etoak.crawl.util.ToolUtil;
...
//		List<String[]> changciList = PageElement.getChangci();
...
//			changci[1]="http://www.huayicang.com/hxgq/xpai/index.jsp?G="+x[0];
			String[] changci = new String[3];
			changci[0]="";
			changci[1]="http://www.huayicang.com/hxgq/xpai/index.jsp?G="+string;
			changci[2]="";
...
//				Goodinfo.addMatchesinfo(matches);
				...
				String dealDate = DateUtil.getStringDate1();
				if(strings[0]!=""){
					String month = strings[0].substring(strings[0].indexOf("月")-2,strings[0].indexOf("月")).trim();
					String day = strings[0].substring(strings[0].indexOf("月")+1,strings[0].indexOf("日")).trim();
					System.out.println(">>>>>"+strings[0]);
					System.out.println(month+">>>"+day);
					String year = DateUtil.getYear();
					dealDate = year+"/"+month+"/"+day;
					System.out.println(DateUtil.strToDate1(dealDate).toString());
				}
				...
				if (Integer.parseInt(pageno) >= 1) {
					...
					for (int pageindex = 1; pageindex <= Integer.parseInt(pageno); pageindex++) {
						...
						String url = "http://www.huayicang.com/hxgq/xpai/index.jsp?P=" + pageindex + "&G=" + matchNO;
						pageMatches = RequestAndResponseTool.sendRequstAndGetResponse(url);
						// 取制定URL的场次
						if (pageMatches.getUrl().indexOf("http://www.huayicang.com/hxgq/xpai/index.jsp") != -1) {
							...
							for (String string : goodList) {
								if (filter.accept(string)) {
									...
									String gname = els.text().substring(els.text().indexOf("：") + 2, els.text().indexOf("[") - 3);
									if(gname.indexOf("咸丰")==-1){
										continue;
									}									
									// 保存商品信息
									...
									try {
										...
										ExcelUtil.insertData(matchNO, good,index);
									} catch (Exception e) {
										e.printStackTrace();
									}
...
//		good.setStprice(elcstartPrice.text().substring(0, elcstartPrice.text().indexOf("(") - 1));
		good.setStprice("￥"+elcsprice.get(0).text());
