...
public class ExcelUtil {
	...
	public static String insertData(String fileName, Good goods, int index) throws Exception {
		File file = new File(fileName + ".xls");
		...
		try {
			// 添加形制
			// 当千，当五百，当百，当五十，当十，当五
			String[] arry = new String[] { "当四", "当八", "当十", "当五十", "当百", "当五百", "当五", "当千" };
			for (String string : arry) {
				if (goods.getGname().indexOf(string) != -1) {
		...
		try {
			// 添加直径MM
			if (goods.getDescp().indexOf("）mm") != -1) {
				System.out.println(">>>>" + goods.getDescp());
				String x = "";
				x = goods.getDescp().substring(goods.getDescp().indexOf("（") + 1, goods.getDescp().indexOf("~"));
				System.out.println(">>>>" + x);
				label = new Label(15, i, x);
			} else if (goods.getDescp().indexOf("）*") != -1) {
				String x = "";
				x = goods.getDescp().substring(goods.getDescp().indexOf("（") + 1, goods.getDescp().indexOf("~"));
				System.out.println(">>>>" + x);
				label = new Label(15, i, x);
				...
				System.out.println(">>>>" + x);
				label = new Label(15, i, x);
			}else if (goods.getDescp().indexOf("mm") != -1) {
				String x = "";
				x = goods.getDescp().substring(0, goods.getDescp().indexOf("*"));
				System.out.println(">>>>" + x);
				label = new Label(15, i, x);
			} else {
		...
		try {
			if (goods.getDescp().indexOf("）mm") != -1) {
				String x = "";
				x = goods.getDescp().substring(goods.getDescp().lastIndexOf("（") + 1,
						goods.getDescp().lastIndexOf("~"));
				System.out.println(">>>>" + x);
				label = new Label(16, i, x);
			} else if (goods.getDescp().indexOf("mm") != -1) {
				String x = "";
				x = goods.getDescp().substring(goods.getDescp().indexOf("*") + 1, goods.getDescp().indexOf("mm"));
				System.out.println(">>>>" + x);
				label = new Label(16, i, x);
			} else {
		...
		// 添加重量G
		try {
			if (goods.getDescp().indexOf("g") != -1) {
				String x = "";
				x = goods.getDescp().substring(goods.getDescp().lastIndexOf("g") - 5,
						goods.getDescp().lastIndexOf("g"));
				System.out.println(">>>>" + x);
				label = new Label(17, i, x);
			} else {
	...
	public static void main(String[] args) {
		try {
			for (int i = 0; i < 10; i++) {
				...
				ExcelUtil.insertData("test", goods, x);
			}
		} catch (IOException e) {
			// TODO 自动生成的 catch 块
			e.printStackTrace();
