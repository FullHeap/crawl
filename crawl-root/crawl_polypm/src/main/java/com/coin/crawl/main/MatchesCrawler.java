...
//		goodNo="00000001";
		...
		for (String[] strings : changciList) {
			if (filter.accept(strings[1])) {
				...
				System.out.println("场次编号为：" + strings[1].substring(strings[1].indexOf("PZ"), strings[1].indexOf("PZ")+9));
...
//				Goodinfo.addMatchesinfo(matches);
				...
				String pageno="1";
				if(elp.size()>0){
					pages = elp.get(0).text();
					pageno = pages.substring(pages.lastIndexOf("共") + 1, pages.lastIndexOf("页")).trim();
