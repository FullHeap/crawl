...
public class DocElement {
	 ...
	 * @return <br>
	 */
	public static List<String[]> getChangci(Document doc) {

		List<String[]> changCiList = new ArrayList<String[]>();

		// 对DIV进行处理： 访问DOM的某个标签
		Elements ediv = DocParserTool.getElementsByClass(doc, "sright");
		if (!ediv.isEmpty()) {
			System.out.println("下面将打印所有sright标签中的链接： ");
			// 取得场次相关信息
			for (Element element : ediv) {
				...
				if (elink.hasAttr("href")) {
					for (Element element2 : elink) {
						// 取得所有场次链接
						// System.out.println("链接为：" +
						// element2.attr("abs:href"));
						// 取得所有场次名称
						// System.out.println("名称为：" + element2.text());

						changci[0] = element2.text();// 名称
						changci[1] = element2.attr("abs:href").replace("..", "hxgq");// 链接
					}
				}

				// 取得藏品数量相关信息
				Elements eFont = element.getElementsByTag("font");
				changci[2] ="";
				for (Element element2 : eFont) {
					// 取得藏品数量
	 ...
	 * @return <br>
	 */
	public static List<String> getGood(Document doc) {
		List<String> goodLinkList = new ArrayList<String>();
		// 对DIV进行处理： 访问DOM的某个标签
		Element xpailistdiv = DocParserTool.getElementsByid(doc, "xpailist");

		// padding中含有对应的商品详细信息
		Elements paddiv = xpailistdiv.getElementsByClass("padding");
		
//		Set<String> links = new HashSet<>();
		for (Element element : paddiv) {
			Elements elink = element.getElementsByTag("a");
			for (Element element2 : elink) {
				// System.out.println("链接为：" + element2.attr("abs:href"));
				// 若链接中含有goods 则代表是商品
				if (element2.attr("abs:href").indexOf("goods") != -1) {
